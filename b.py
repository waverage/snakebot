...
def train(env, replay_memory, model, target_model, done, steps):
    ...
    for index, (observation, action, reward, new_observation, done) in enumerate(mini_batch):
        ...
        print("\n====================")
        print("done", done)
        print("(1 - learning_rate) * current_qs[action]", (1 - learning_rate) * current_qs[action])
        print("learning_rate * max_future_q", learning_rate * max_future_q)
        old_qs = current_qs[action]
        current_qs[action] = (1 - learning_rate) * current_qs[action] + learning_rate * max_future_q
        print("current action q", old_qs, "new q:", current_qs[action], "diff:", current_qs[action] - old_qs)

        print("reward", reward)
        print("max_future_q", max_future_q)
        #if old_qs > current_qs[action]:
        if steps > 100 and max_future_q < 1.1:
    ...
    for episode in range(train_episodes):
        ...
        while not done:
            ...
            if random_number <= epsilon:
                ...
                tensor = tf.convert_to_tensor(encoded_reshaped, np.float32)
